...
//var type = document.querySelector('input[name="group22"]:checked');
//var type = $('input[name=group22]:checked', '#reviewForm');
var type = document.getElementById("gp");
var hiddenId = document.getElementById('hiddenId');

reviewForm.addEventListener('submit', (e) => {
    e.preventDefault();

    if (!nom.value || !tel.value || !adresse.value || !lat.value || !lng.value || !type.value || !specialite.value) return null

    var id = hiddenId.value || Date.now()

    db.ref('ehealth/hopitaux/' + id).set({
        nom: nom.value,
        tel: tel.value,
...
var reviewsRef = db.ref('ehealth/hopitaux');
...
reviews.addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('edit')) {
        ...
        specialite.value = reviewNode.querySelector('.specialite').innerText;
        type.value = reviewNode.querySelector('.type').innerText;
    ...
    if (e.target.classList.contains('delete')) {
        ...
        db.ref('ehealth/hopitaux/' + id).remove();

    }
});

function reviewTemplate({ nom, tel, adresse, lat, lng, type, specialite }) {
    return `
    ...
    <td class="lat">${lat}</td>
    <td class="lng">${lng}</td>
    <td class="specialite">${specialite}</td>
